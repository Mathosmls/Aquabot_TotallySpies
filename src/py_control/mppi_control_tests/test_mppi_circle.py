...
class MPPIControllerForAquabot:
    def __init__(
            ...
            number_of_samples_K: int = 400,
            sigma: np.ndarray = np.array([500,500,0.15,0.15]),
            param_lambda: float = 1.0,
            stage_cost_weight: np.ndarray = np.array([0.01, 8.0,30.0]),  # poids pour [x, y, theta]
    ...
    def _apply_acceleration_limit(self, u: np.ndarray, u_prev: np.ndarray) -> np.ndarray:
        """
        Applique une limite d'accélération aux commandes en tenant compte de l'intervalle de temps.
        
        :param u: Commande actuelle [v, omega].
        :param u_prev: Commande précédente [v_prev, omega_prev].
        :param dt: Intervalle de temps entre les mises à jour.
        :return: Commande limitée [v_limited, omega_limited].
        """
        # Calcul de la variation de commande divisée par l'intervalle de temps pour obtenir une accélération
        ...
        # Recalculer u en tenant compte des limitations d'accélération
    ...
    def _dynamics(self, state: np.ndarray, control: np.ndarray) -> np.ndarray:
        ...
        # Cache constants to avoid repeated access to object attributes
        m = self.m
        xU = self.xU
        xUU = self.xUU
        yV = self.yV
        yVV = self.yVV
        pos_mot_y = self.pos_mot_y
        pos_mot_x = self.pos_mot_x
        nR = self.nR
        nRR = self.nRR
        Iz = self.Iz
        dt = self.dt
    ...
    def _stage_cost(self, state: np.ndarray, target: np.ndarray) -> float:
        ...
        return (self.stage_cost_weight[0] * self.huber_loss(0.5/v,1.0) +
                self.stage_cost_weight[1] * self.huber_loss(10-d_o,3.0)  +
    ...
    def _moving_average_filter(self, xx: np.ndarray, window_size: int) -> np.ndarray:
        # Utilisation de np.convolve pour appliquer la moyenne mobile avec mode='same'
...
def run_simulation_mppi_differential_drive():
    ...
    horizon_step_T = 15
    sim_steps = 10  # Nombre total d'étapes de simulation

    # Initialisation de l'état du véhicule et du contrôleur
    initial_state = np.array([7.0, 0.0, 3.1, 0.0,0.0,0.0])  # Démarre au bas du cercle
