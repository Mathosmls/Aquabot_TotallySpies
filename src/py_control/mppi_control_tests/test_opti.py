...
def stage_cost(state: np.ndarray, target: np.ndarray, stage_cost_weight: np.ndarray) -> float:
    ...
    #         stage_cost_weight[1] * huber_loss(25-y, 5.0) +
    #         stage_cost_weight[2] * (theta - theta_t)**2)
    return (stage_cost_weight[0] * huber_loss(0.5/v, 5.0) +
            stage_cost_weight[1] * huber_loss(10-d_o, 5.0) +
            stage_cost_weight[2] * (theta - theta_t)**2)
...
def run_simulation_mppi_differential_drive():
    ...
    # horizon_step_T = 25
    sim_steps = 500  # Nombre total d'étapes de simulation
    ...
    controller = MPPIControllerForAquabot()
