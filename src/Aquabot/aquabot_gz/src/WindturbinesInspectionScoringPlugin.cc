...
class WindturbinesInspectionScoringPlugin::Implementation
{
  ...
  public: double aisWindturbinesPeriod = 10.0;
  ...
  sim::EntityComponentManager &_ecm, sim::EventManager &_eventMgr)
{
  ...
  this->dataPtr->aisWindturbinesPositionsPub = this->dataPtr->node.Advertise<msgs::Pose_V>(
...
void WindturbinesInspectionScoringPlugin::PreUpdate( const sim::UpdateInfo &_info, sim::EntityComponentManager &_ecm)
{
  ...
  {
    // Windturbines AIS
    msgs::Pose_V windturbinesPosesMsg;
    for(auto windturbinePose : currentData.windturbines)
    {
      math::Vector3d cartVec(-windturbinePose.Pos().X(), -windturbinePose.Pos().Y(), windturbinePose.Pos().Z());
      math::Vector3d latlon = this->dataPtr->sc.SphericalFromLocalPosition(cartVec);
      const math::Quaternion<double> orientation = windturbinePose.Rot();
      math::Pose3d pose(latlon, orientation);
      msgs::Set(windturbinesPosesMsg.add_pose(),pose);
    }
    this->dataPtr->aisWindturbinesPositionsPub.Publish(windturbinesPosesMsg);
